...
async function fetchJobListings() {
  console.log(`Fetching METRO jobs page: ${TARGET_URL}`);
...
function parseJobsHTML(html) {
  ...
  $(".attrax-vacancy-tile").each((_, el) => {
    const $el = $(el);

    const title = $el.find(".attrax-vacancy-tile__title").first().text().trim();
    if (!title) return;

    const locationEl = $el.find(".attrax-vacancy-tile__location-freetext .attrax-vacancy-tile__item-value").first();
