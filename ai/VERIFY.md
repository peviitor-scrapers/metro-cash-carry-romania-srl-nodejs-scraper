...
| `job-deep-validate.yml` | `workflow_dispatch` | Validare deep via Playwright (browser mode) |
| `job-recovery-from-disaster.yml` | `workflow_dispatch` | Recuperare date (scrape forțat + re-upsert) |
...
4. Verifică prin API că job-urile companiei apar:
   ```bash
   curl -s "https://api.peviitor.ro/v1/scraper/jobs/?cif=8119423&rows=10" | jq
   ```
5. Verifică că `docs/jobs.md` a fost generat și este accesibil:
   - https://peviitor-scrapers.github.io/metro-cash-carry-romania-srl-nodejs-scraper/jobs.md
