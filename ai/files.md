...
| File | Description |
|------|-------------|
| `scraper/index.js` | Main scraper - full workflow: validate company → scrape METRO HTML → transform → upsert → generate docs/jobs.md. Exports `parseJobsHTML`, `mapToJobModel`, `transformJobsForSOLR` |
...
| `scraper/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), validateByBrowser(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by `validate-jobs.js`, `tests/validate-metro-jobs.js`, and the deep-validate workflow. |
...
| File | Description |
|------|-------------|
| `scraper/config/company.json` | **Single source of truth for company identity.** All scraper code, CI workflows, and the static HTML read from this file. `scraperFile` must be the GitHub Actions workflow URL (not raw). |
| `scraper/config/company.js` | ESM wrapper that imports and exposes `scraper/config/company.json` to Node code |
| `scraper/config/scraper.json` | Scraper-specific config - apiBase (cariere.metro.ro), apiCountryId |
...
| `tests/validate-metro-jobs.js` | **METRO-specific validator (used by CI).** Modes: `--head` (default), `--content`, `--browser` (Playwright). Called nightly by `automation-testing.yml` and manually via `job-deep-validate.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real METRO careers page, ANAF, and Peviitor API |
...
| `tests/consistency/root-files.test.js` | Verifies required root files exist and match expectations |
| `tests/consistency/topics.test.js` | Verifies repository has required topics: job-seeker-ro-spider, peviitor-ro |
| `tests/consistency/version.test.js` | Verifies package.json and CHANGELOG.md versions are in sync |
...
| `ai/ROBOTS.md` | robots.txt analysis and scraping policy for METRO Careers (cariere.metro.ro) |
...
| `ai/MAINTENANCE.md` | Maintenance agent workflow - issues, schemas, workflows, cleanup |

> Note: `SECURITY.md`, `CODE_OF_CONDUCT.md`, and `CONTRIBUTING.md` live at the repository root. The EPAM template also ships `ai/AI-DERIVATION-GUIDE.md` — derived scrapers **do not** carry it; see the EPAM template for deriving new scrapers.
...
| `.github/workflows/job-recovery-from-disaster.yml` | Manual data recovery (forced scrape + re-upsert) |
| `.github/workflows/automation-template-sync-check.yml` | Weekly check that derived scrapers are up to date with the EPAM template |
...
- Full workflow: validate company (ANAF+CUIScan+CUIFirma+Peviitor) → scrape METRO HTML (cheerio) → transform → upsert → generate docs/jobs.md
