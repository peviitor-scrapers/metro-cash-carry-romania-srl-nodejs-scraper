...
This scraper extracts job listings from METRO careers page (Romania only) and imports them to peviitor.ro.

Target: https://cariere.metro.ro/jobs
...
- **Node.js & JavaScript** - For scraping and data extraction (cheerio)
- **Peviitor API** - For data storage and retrieval (api.peviitor.ro)
- **Claude Code** - For development

## Workflow Steps

1. **Start with brand** - We know the brand ("METRO")
...
8. **Scrape new jobs** - Extract jobs from METRO careers HTML page (Romania)
...
# Test mode (no ANOFM merge)
node scraper/index.js --test
```

> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts METRO Careers jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from METRO careers HTML page (Romania only) + ANOFM merge
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape METRO HTML (jobs for Romania)
    │
    ▼
searchANOFM(CIF) ──► merge ANOFM jobs (non-test mode)
    │
    ▼
...
| `scraper/config/scraper.json` | Scraper-specific config (apiBase, apiCountryId) |
| `scraper/index.js` | Main entry point - full workflow: validate company → scrape HTML → transform → upsert → delete stale → generate docs/jobs.md |
...
| `scraper/validate-jobs.js` | Generic deep validator (manual use) - full GET requests, content-aware |
...
| `tests/validate-metro-jobs.js` | METRO-specific validator (used by CI) - HEAD/content/browser modes |
...
- **ANOFM**: `https://mediere.anofm.ro/api/entity/vw_public_job_posting` - POST, search jobs by employer tax code
- **Peviitor API**: `https://api.peviitor.ro/v1/` — all job and company operations go through this API
- **METRO Careers page**: `https://cariere.metro.ro/jobs` — GET, HTML parsed with cheerio (`.attrax-vacancy-tile`)
...
1. **Unit Tests** - Test individual modules (api.js, company.js, anaf.js, index.js) in isolation
...
4. **Consistency Tests** - Test GitHub repo config (public, topics, workflows, versions)
...
- [x] Migrate from SOLR to Peviitor API (#6)
- [x] Restructure into `scraper/` layout matching the EPAM template (#7)
