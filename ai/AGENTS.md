...
```bash
gh run view <RUN_ID> --repo peviitor-scrapers/metro-cash-carry-romania-srl-nodejs-scraper --json status -q .status
```
...
**On every session:**
1. Check open GitHub issues: `gh issue list --repo peviitor-scrapers/metro-cash-carry-romania-srl-nodejs-scraper --state open`
