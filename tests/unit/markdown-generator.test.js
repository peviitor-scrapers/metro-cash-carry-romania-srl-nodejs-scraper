...
const baseCompany = {
  ...
  career: ["https://cariere.metro.ro/jobs"],
  lastScraped: "2026-06-17"
};

const baseJob = {
  url: "https://cariere.metro.ro/job/test-bucuresti-jid-123",
...
describe("generateJobsMarkdown", () => {
  describe("company section", () => {
    it("includes company name as h1", () => {
      ...
      expect(md).toContain("# METRO CASH & CARRY ROMANIA SRL");
    });

    it("includes CIF", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("8119423");
    });
    ...
    it("includes website as markdown link", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("[https://www.metro.ro](https://www.metro.ro)");
    });

    it("includes career page as markdown link", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("[https://cariere.metro.ro/jobs](https://cariere.metro.ro/jobs)");
    });
    ...
    it("omits optional fields when not present", () => {
      const minimal = { id: "8119423", company: "METRO CASH & CARRY ROMANIA SRL" };
  ...
  describe("jobs section", () => {
    ...
    it("includes job URL as markdown link", () => {
      const md = generateJobsMarkdown(baseCompany, [baseJob]);
      expect(md).toContain("[https://cariere.metro.ro/job/test-bucuresti-jid-123]");
    });
    ...
    it("renders multiple jobs", () => {
      const job2 = { ...baseJob, title: "DevOps Engineer", url: "https://cariere.metro.ro/job/test-cluj-jid-456" };
    ...
    it("handles job with no optional fields", () => {
      const minimal = { url: "https://cariere.metro.ro/job/test-jid-999", title: "QA Engineer" };
